...
║                                                             ║
║  Usage:                                                     ║
║    python warden.py              # Run collector loop       ║
║    python warden.py --once       # Single capture & exit    ║
║    python warden.py --export     # Export payload to JSON   ║
║    python warden.py --cleanup    # Run janitor manually     ║
║    python warden.py --setup      # Create DB table          ║
╚══════════════════════════════════════════════════════════════╝
"""
...
def cmd_run():
    ...
    Runs janitor once per day at midnight.
    """
    ensure_table()
    ...
    while _running:
        ...
        if last_cleanup_date != today:
            try:
                ...
                logger.info("Daily janitor complete. Deleted: %d", deleted)
            except Exception as exc:
                logger.error("Janitor error: %s", exc, exc_info=True)
...
def cmd_cleanup():
    """Manual janitor run."""
    deleted = cleanup()
    print(f"Janitor: {deleted} rows deleted.")
...
def main():
    ...
    group.add_argument("--cleanup", action="store_true", help="Run janitor")
