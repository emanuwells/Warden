...
| Warden Clean produção (cron) | `bash scripts/warden_clean.sh` — lê `WARDEN_CLEAN_*` do `.env` |
| Arquivo semanal prune | `.venv/bin/python scripts/weekly_archive.py --prune-only` |
