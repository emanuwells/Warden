...
function warden_app_version(): string
{
    ...
    $fromEnv = trim((string)(getenv('WARDEN_VERSION') ?: ''));
    if ($fromEnv !== '') {
        $cached = $fromEnv;
        return $cached;
    }

    foreach ([dirname(__DIR__, 3) . '/VERSION', dirname(__DIR__, 4) . '/VERSION'] as $path) {
        if (!is_readable($path)) {
            continue;
        }
...
function warden_envelope(string $kind, array $payload, array $meta): array
{
    ...
    return [
        'ok' => true,
        'kind' => $kind,
        'app_version' => warden_app_version(),
...
function warden_dev_skip_auth_enabled(): bool
{
    $flag = strtolower(trim((string)(getenv('WARDEN_DEV_SKIP_AUTH') ?: '')));
    return in_array($flag, ['1', 'true', 'yes', 'on'], true);
}

function warden_require_auth(): array
{
    if (warden_dev_skip_auth_enabled()) {
        return [
            'username' => 'local-dev',
            'appRole' => 'admin',
            'globalTier' => 'admin',
            'userType' => 'standard',
            'permissionSource' => 'dev_skip_auth',
        ];
    }
